...
export function parseFrontmatter(text) {
    ...
    for (const line of text.slice(4, end).split("\n")) {
        ...
        if (!m) {
            throw new CairnError("CONFIG_INVALID", `unsupported frontmatter line: ${line} (flat key: value or key: [a, b] only)`);
        }
        const [, key, raw] = m;
        if (raw.startsWith("[")) {
            if (!raw.endsWith("]")) {
...
export function serializeFrontmatter(data, body) {
    const lines = Object.entries(data).map(([k, v]) => Array.isArray(v) ? `${k}: [${v.join(", ")}]` : `${k}: ${v}`);
    return `---\n${lines.join("\n")}\n---\n${body}`;
}
...
export function parsePlanDoc(text) {
    const { data, body } = parseFrontmatter(text);
    const known = { issues: data.issues, depth: data.depth };
    const result = PlanFrontmatterSchema.safeParse(Object.fromEntries(Object.entries(known).filter(([, v]) => v !== undefined)));
    if (!result.success) {
