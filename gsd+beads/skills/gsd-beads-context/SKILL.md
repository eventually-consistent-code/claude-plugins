---
name: gsd-beads-context
description: Use when a repo has .gsd-beads/context.json AND the context-mode ctx_* MCP tools are available — makes context-mode's knowledge base GSD/beads-aware. Scopes indexed memory by the active bd issue + GSD phase (source-label convention), recalls only the active task's context, and watches ctx_stats to advise task-splitting before the window degrades. Scope-by-label only — never deletes. Complements the gsd-beads skill.
---
...
- `.planning/` (GSD) and `.beads/` (beads) both exist — the `gsd-beads` skill is active.
- `.gsd-beads/context.json` exists with `"enabled": true` — the user opted in.
- The `ctx_*` MCP tools (context-mode) are available in the session.

If `.gsd-beads/context.json` is missing, the user has not opted in — do **not**
create it implicitly; point them at `/gsd-beads:context-config`. If the `ctx_*`
...
`scoping.source_template` in `.gsd-beads/context.json` (default
`gb/{bd_id}/{phase}`):
...
## Lifecycle — hook into the gsd-beads flow

Run these alongside the `gsd-beads` lifecycle (claim → in_progress → close):

| gsd-beads event | context action |
|---|---|
...
When `capacity_guard.enabled` is true in `.gsd-beads/context.json`:
...
- On any conflict between a bd issue and GSD phase docs, **GSD docs win** —
  identical to the `gsd-beads` skill.
- If `.gsd-beads/context.json` is absent or `enabled:false`, do nothing here;
